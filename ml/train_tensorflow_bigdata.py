...
``spark/train_driver_win_mllib.py`` and trains a neural network using
TensorFlow's ``tf.data`` API together with ``tf.distribute`` so it can scale
from a single machine to a multi-worker cluster without code changes. The
input reader streams CSV shards directly from disk, making it suitable for
large datasets stored on shared volumes or cloud object storage.
"""
...
def _feature_store_glob() -> str:
    ...
    if not tf.io.gfile.exists(csv_dir):
        raise FileNotFoundError(
            "Spark feature store shards not found. Run train_driver_win_mllib.py first or "
            f"set F1_FEATURE_STORE_PATH to a directory containing CSV shards (missing: {csv_dir})."
        )
...
def _make_base_dataset(batch_size: int) -> tf.data.Dataset:
    ...
    dataset = dataset.enumerate()  # attach batch indices for deterministic splitting
...
def _build_model() -> tf.keras.Model:
    ...
    model.compile(
        ...
        metrics=[tf.keras.metrics.AUC(name="roc_auc"), tf.keras.metrics.Precision(), tf.keras.metrics.Recall()],
    )
    return model
...
def main() -> None:
    ...
    for name, dataset in (("train", train_ds), ("val", val_ds), ("test", test_ds)):
        ...
        if cardinality == tf.data.experimental.UNKNOWN_CARDINALITY:
            raise RuntimeError(f"Unable to determine cardinality for {name} dataset. Check feature shards.")
        if cardinality.numpy() == 0:
            raise RuntimeError(f"{name} dataset is empty. Ensure Spark feature export produced enough batches.")
    ...
    model, history, test_metrics = _train_and_evaluate(strategy, train_ds, val_ds, test_ds, epochs)
